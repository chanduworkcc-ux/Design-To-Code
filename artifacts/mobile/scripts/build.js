...
// Use a dedicated build port that doesn't conflict with the mockup-sandbox
// (which occupies 8081) or the dev proxy (which occupies 18115/18200).
// Override with METRO_PORT env var if needed.
const _metroPortRaw = process.env.METRO_PORT;
const _metroParsed = _metroPortRaw ? parseInt(_metroPortRaw, 10) : NaN;
const METRO_PORT =
  !_metroPortRaw || !/^\d+$/.test(_metroPortRaw.trim()) || !Number.isInteger(_metroParsed) || _metroParsed < 1 || _metroParsed > 65535
    ? 18201
    : _metroParsed;
if (_metroPortRaw && METRO_PORT === 18201 && _metroPortRaw.trim() !== "18201") {
  console.warn(`[build] Invalid METRO_PORT "${_metroPortRaw}" — falling back to 18201`);
}
